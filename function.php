...
function base_url()
{
	return 'http://localhost/Apto7/libraryproyect/';
}
...
//Monedas para las opciones (de internet)
	...
	function fill_author($connect)
{
	...
	$output = '<option value="">Selección de autor</option>';
...
function fill_category($connect)
{
	...
	$output = '<option value="">Selección de categoría</option>';
...
function fill_location_rack($connect)
{
	...
	$output = '<option value="">Seleción de anaquel</option>';
...
function Count_total_fines_user($connect,$user)
{
	$total = 0;

	$query = "
	SELECT SUM(book_fines) AS Total FROM lms_issue_book 
	WHERE NOT book_issue_status = 'Return' AND lms_issue_book.user_id = '".$user."'
