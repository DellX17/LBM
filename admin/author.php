...
<div class="container-fluid py-4" style="min-height: 700px;">
	...
	<ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
        ...
        <li class="breadcrumb-item"><a href="author.php">Administrar Autores</a></li>
    		...
    		<div class="card mb-4">
                ...
                <div class="card-body">
                	<form method="post">
                		...
                		<div class="mt-4 mb-0">
                			<input type="submit" name="add_author" class="btn btn-success" value="Nuevo" />
                		</div>
                	</form>
                </div>
            </div>
    			...
    			<div class="card-body">
    				<form method="post">
    					...
    					<div class="mt-4 mb-0">
    						...
    						<input type="submit" name="edit_author" class="btn btn-primary" value="Editar" />
    					</div>
    				</form>
    			</div>
    		</div>
	...
	<ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
		<li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
		<li class="breadcrumb-item active">Administrar Autores</li>
	</ol>
	<?php 
	...
	if(isset($_GET["msg"]))
	{
		if($_GET["msg"] == 'add')
		{
			echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Nuevo autor añadido!<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
		}
		if($_GET['msg'] == 'edit')
		{
		...
		if($_GET["msg"] == 'disable')
		{
			echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Autor deshabilitado <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
		}

		if($_GET["msg"] == 'enable')
		{
			echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Autor habilitado <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
		}
	}
		...
		<div class="card-header">
			<div class="row">
				<div class="col col-md-6">
					<i class="fas fa-table me-1"></i> Adminsitrar Autores
				</div>
				<div class="col col-md-6" align="right">
					<a href="author.php?action=add" class="btn btn-success btn-sm">Nuevo</a>
				</div>
			</div>
		</div>
		<div class="card-body">
			<table id="datatablesSimple">
				...
				if($statement->rowCount() > 0)
				{
					foreach($statement->fetchAll() as $row)
					{
						...
						{
							$author_status = '<div class="badge bg-success">Habilitado</div>';
						}
						else
						{
							$author_status = '<div class="badge bg-danger">Deshabilitado</div>';
						}
						...
						<tr>
							...
							<td class= "text-right">
								<a href="author.php?action=edit&code='.convert_data($row["author_id"]).'" class="btn btn-sm btn-primary">Editar</a>
								<button type="button" name="delete_button" class="btn btn-danger btn-sm" onclick="delete_data(`'.$row["author_id"].'`, `'.$row["author_status"].'`)">Habilitado</button>
							</td>
						</tr>
						';
					}
